...
VertexStruct main_vertex(float4 attrib[5] : ATTRIB, uint vid : SV_VertexID)
{
	float2 p;
...
float4 main_pixel(VertexStruct vresult) : SV_Target
{
...
technique Slug {
    pass {
        VertexShader = compile VS_SHADERMODEL main_vertex();
        PixelShader = compile PS_SHADERMODEL main_pixel();
    }
}